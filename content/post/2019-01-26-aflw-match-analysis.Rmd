...
Again, the `View()` and `str()` functions are helpful for initially inspecting the data set so you can take a look at its structure and contents.

## Data preparation

Some data wrangling is needed to modify the data structure to suit this analysis. The code below results in a data frame where every match has two rows, with one row for the home team's game statistics and one row for the away team's game statistics.

``` {r prepare_match_data, message = FALSE}

# Wrangle match summary data
